...
static int CommandHandle(char* Command);
...
static const char * const clearScreen = "\x1b[2J";
...
portBASE_TYPE xReturned;
	...
	prvConsoleConfigureUART( USART2 );
	...
	prvUARTSendBuffer( USART2, clearScreen, strlen( clearScreen ) );
	prvUARTSendBuffer( USART2, pcWelcomeMessage, strlen( pcWelcomeMessage ) );

	for( ;; )
	{
		/* Wait for the next character to arrive */
		cRxedChar = usart_recv_blocking( USART2 );
			/* Echo the character back. */
			prvUARTSendBuffer( USART2, &cRxedChar, sizeof( cRxedChar ) );

			/* Was it the end of the line? */
			if( cRxedChar == '\n' || cRxedChar == '\r' )
			{
				/* Just to space the output from the input. */
				prvUARTSendBuffer( USART2, pcNewLine, strlen( pcNewLine ) );
				...
				do
				{
					...
					prvUARTSendBuffer( USART2, pcOutputString, strlen( pcOutputString ) );
				...
				processed again. */
				if(!CommandHandle(pcOutputString))
					strcpy( cLastInputString, cInputString );
				else if(strcmp(cInputString, "help")){
					prvUARTSendBuffer( USART2, clearScreen, strlen( clearScreen ) );
					prvUARTSendBuffer( USART2, pcNewLine, strlen( pcNewLine ) );
					prvUARTSendBuffer( USART2, pcOutputString, strlen( pcOutputString ) );
				}
				...
				prvUARTSendBuffer( USART2, pcEndOfOutputMessage, strlen( pcEndOfOutputMessage ) );
			}
			else
			{
				if( cRxedChar == '\r' )
...
static int CommandHandle(char* Command)
{
	...
	if(strstr(Command,"Command") == Command){
		switch(Command[8])
		{
			case '1':
			{
